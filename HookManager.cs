...
namespace TerrariaApi.Server
{
	...
	public class HookManager
	{
		...
		{
			Console.WriteLine("TerrariaAPI Version: {0} (Protocol {1} ({2}), tModLoader: {3})",
				ServerApi.ApiVersion,
				Main.versionNumber2,
				Main.curRelease,
				BuildInfo.tMLVersion);
		}
					...
					switch ((PacketTypes)msgId)
				{
					...
					case PacketTypes.ClientUUID:
						if (length == 38)
						{
							...
							{
								var result = SHA512.HashData(uuid);
								ServerApi.ClientUUID[buffer.whoAmI] = result.Aggregate("", (s, b) => s + b.ToString("X2"));
								return true;
							}
						}
						ServerApi.ClientUUID[buffer.whoAmI] = "";
						return true;
				}
			}
